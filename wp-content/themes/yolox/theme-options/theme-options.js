...
jQuery( document ).ready(
	function() {
        ...
        jQuery( window ).load(
            function() {
                jQuery( '.yolox_options .yolox_options_section' ).each(
                    function () {
		...
		jQuery( '.yolox_options .yolox_options_item_field [name^="yolox_options_field_"]' ).on(
			'change', function () {
		...
		function yolox_options_check_dependencies(cont) {
			cont.find( '.yolox_options_item_field' ).each(
				function() {
					...
					var depend = false;
					for (var fld in yolox_dependencies) {
						if (fld == id) {
					...
					if (depend) {
						...
						for (var i in depend) {
							...
							if (name.charAt( 0 ) == '#' || name.charAt( 0 ) == '.') {
								fld = jQuery( name );
								if (fld.length > 0 && ! fld.hasClass( 'yolox_inited' )) {
									fld.addClass( 'yolox_inited' ).on(
										'change', function () {
											jQuery( '.yolox_options .yolox_options_section' ).each(
												function () {
													yolox_options_check_dependencies( jQuery( this ) );
						...
						if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
							ctrl.parents( '.yolox_options_item' ).show().removeClass( 'yolox_options_no_use' );
						} else {
							ctrl.parents( '.yolox_options_item' ).hide().addClass( 'yolox_options_no_use' );
						}
					}
