	...
	function yolox_customizer_css_template() {
		...
		if ( count( $schemes ) > 0 ) {
			foreach ( $schemes as $scheme ) {
				yolox_show_layout(
					...
					'<' . esc_html( $tmpl_holder ) . ' type="text/html" id="tmpl-yolox-color-scheme-' . esc_attr( $scheme ) . '">',
		...
		if ( is_array( $fonts ) && count( $fonts ) > 0 ) {
			...
			yolox_show_layout(
				...
				'<' . esc_html( $tmpl_holder ) . ' type="text/html" id="tmpl-yolox-fonts">',
				'</' . esc_html( $tmpl_holder ) . '>'
			);
		}
		...
		if ( is_array( $vars ) && count( $vars ) > 0 ) {
			...
			yolox_show_layout(
				...
				'<' . esc_html( $tmpl_holder ) . ' type="text/html" id="tmpl-yolox-vars">',
				'</' . esc_html( $tmpl_holder ) . '>'
			);
		}
