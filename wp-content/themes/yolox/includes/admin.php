...
if ( ! function_exists( 'yolox_admin_scripts' ) ) {
		...
		if ( $all || is_object( $screen ) ) {
			if ( $all || yolox_options_allow_override( ! empty( $screen->post_type ) ? $screen->post_type : $screen->id ) ) {
				// Load font icons
				wp_enqueue_style( 'yolox-icons', yolox_get_file_url( 'css/font-icons/css/fontello-embedded.css' ), array(), null );
				wp_enqueue_style( 'yolox-icons-animation', yolox_get_file_url( 'css/font-icons/css/animation.css' ), array(), null );
				// Load theme fonts
