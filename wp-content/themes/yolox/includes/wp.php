...
if ( ! function_exists( 'yolox_get_nav_menu' ) ) {
	function yolox_get_nav_menu( $location = '', $menu = '', $depth = 11, $custom_walker = false ) {
		static $list = array();
		$class       = '';
			...
			foreach ( $terms as $term ) {
				...
				$output .= ( $links
									? '<a href="' . esc_url( get_term_link( $term->term_id, $taxonomy ) ) . '"'
											...
											. ' title="' . sprintf( esc_attr__( 'View all posts in %s', 'yolox' ), $term->name ) . '"'
											. '>'
									: '<span>'
								)
...
if ( ! function_exists('yolox_get_privacy_text' ) ) {
	function yolox_get_privacy_text() {
		...
		return apply_filters( 'yolox_filter_privacy_text', wp_kses_post( 
			$privacy_text
