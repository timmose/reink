...
$GLOBALS['YOLOX_STORAGE'] = array(
	...
	'required_plugins'   => array_merge(
		...
		array(
			...
			'wp-gdpr-compliance' => esc_html__( 'WP GDPR Compliance', 'yolox' ),
	...
	'theme_privacy_url'  => '//yolox.ancorathemes.com/privacy-policy/',                   // Ancora
		...
		if ( is_array( $options ) ) {
			...
			$options['banners'] = array(
				array(
					...
					'content'      => wp_kses_post( __( "Create your front page right in the WordPress Customizer. There's no need in any page builder. Simply enable/disable sections, fill them out with content, and customize to your liking.", 'yolox' ) ),
				...
				array(
					...
					'content'      => wp_kses_post( __( 'Use Layouts Builder to create and customize header and footer styles for your website. With a flexible page builder interface and custom shortcodes, you can create as many header and footer layouts as you want with ease.', 'yolox' ) ),
				...
				array(
					...
					'content'      => wp_kses_post( __( 'Need more details? Please check our full online documentation for detailed information on how to use Yolox.', 'yolox' ) ),
				...
				array(
					...
					'content'      => wp_kses_post( __( 'No time for reading documentation? Check out our video tutorials and learn how to customize Yolox in detail.', 'yolox' ) ),
				...
				array(
					...
					'content'      => wp_kses_post( __( "Need a website fast? Order our custom service, and we'll build a website based on this theme for a very fair price. We can also implement additional functionality such as website translation, setting up WPML, and much more.", 'yolox' ) ),
...
if ( ! function_exists( 'yolox_ocdi_set_options' ) ) {
	...
	function yolox_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			$options['files']['ocdi']['domain_demo'] = esc_url( yolox_get_protocol() . '://yolox.ancorathemes.com' );
		}
		return $options;
	}
}
		...
		yolox_storage_set(
			'options', array(
				...
				'header_style'                  => array(
					...
					'desc'       => wp_kses_post( __( 'Select custom header from Layouts Builder', 'yolox' ) ),
					'override'   => array(
						'mode'    => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
						'section' => esc_html__( 'Header', 'yolox' ),
					),
				...
				'footer_style'                  => array(
					'title'      => esc_html__( 'Select custom layout', 'yolox' ),
					'desc'       => wp_kses_post( __( 'Select custom footer from Layouts Builder', 'yolox' ) ),
					'override'   => array(
						'mode'    => 'page,cpt_team,cpt_services,cpt_dishes,cpt_competitions,cpt_rounds,cpt_matches,cpt_cars,cpt_properties,cpt_courses,cpt_portfolio',
						'section' => esc_html__( 'Footer', 'yolox' ),
					),
                ...
                'header_style_post'                  => array(
                    'title'      => esc_html__( 'Select custom layout', 'yolox' ),
                    'desc'       => wp_kses_post( __( 'Select custom header from Layouts Builder on the single post', 'yolox' ) ),
                    'dependency' => array(
		...
		foreach ( $theme_fonts as $tag => $v ) {
			$fonts[ "{$tag}_section" ] = array(
				...
				'desc'  => ! empty( $v['description'] )
								...
								: wp_kses_post( sprintf( __( 'Font settings of the "%s" tag.', 'yolox' ), $tag ) ),
				'type'  => 'section',
			);
