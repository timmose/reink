...
if ( ! function_exists( 'yolox_hovers_add_icons' ) ) {
	function yolox_hovers_add_icons( $hover, $args = array() ) {
		...
		if ( in_array( $hover, array( 'icons', 'zoom' ) ) ) {
			...
			?>
			<div class="icons">
				<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php yolox_show_layout($target); ?> aria-hidden="true" class="icon-link
									<?php
			...
			<div class="icons">
						...
						apply_filters(
							...
							'<a rel="nofollow" href="' . esc_url( $product->add_to_cart_url() ) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ( 'shop_buttons' == $hover ? ( $product->is_type( 'variable' ) ? esc_html__( 'Select options', 'yolox' ) : esc_html__( 'Buy now', 'yolox' ) ) : '' )
										. '</a>',
							$product
						)
					);
				...
				if ( 'shop_buttons' == $hover ) {
					if ( is_object( $args['cat'] ) ) {
						esc_html_e( 'View products', 'yolox' );
					} else {
						esc_html_e( 'Details', 'yolox' );
					}
				}
				?>
				</a>
			...
			// Hover style 'Icon'
			?>
			<div class="icons"><a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php yolox_show_layout($target); ?> aria-hidden="true" class="icon-search-alt"></a></div>
			<?php

		} elseif ( 'dots' == $hover ) {
			// Hover style 'Dots'
			?>
			<a href="<?php echo ! empty( $post_link ) ? esc_url( $post_link ) : $no_link; ?>" <?php yolox_show_layout($target); ?> aria-hidden="true" class="icons"><span></span><span></span><span></span></a>
			<?php
