...
if( ! class_exists( 'CSF_Field_typography' ) ) {
  class CSF_Field_typography extends CSF_Fields {
    ...
    public function render() {
        ...
        // Custom
        if( !empty($this->value['font-size']) && strpos( $this->value['font-size'], 'px')){
            $this->value['font-size'] = str_replace('px', '', $this->value['font-size']);
        }
        if( !empty($this->value['subsets']) ){
            $this->value['subset'] = $this->value['subsets'];
        }
